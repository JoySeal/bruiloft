...
function setup() {
  document.addEventListener('click', (e) => {
    ...
    if (clickedCard) {
      if (clickedCard.classList.contains('disabled')) {
        const fullscreenButton = clickedCard.querySelector('.action--buy');
        if (fullscreenButton) {
          fullscreenButton.style.display = 'none';
        }
      } else {
        if (expandedCard || e.target.matches('.action--button')) return;
        ...
        expand();
        history.pushState({ isCardView: true }, null, null);
        e.stopPropagation();
      }
    }
  });
  ...
  const disabledCards = document.querySelectorAll('.js-card.disabled');
  disabledCards.forEach((disabledCard) => {
    const fullscreenButton = disabledCard.querySelector('.action--buy');
    if (fullscreenButton) {
      fullscreenButton.style.display = 'none';
    }
  });
